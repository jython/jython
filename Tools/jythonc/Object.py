...
def PyObjectArray(args):
	aargs = makeAnys(args)
	return Object(jast.FilledArray("PyObject", aargs), findType(java.lang.Class.forName("[Lorg.python.core.PyObject;")))
	
def makeStringArray(args):
	return Object(jast.StringArray(args), findType(java.lang.Class.forName("[Ljava.lang.String;")))
...
primitives = {java.lang.Integer.TYPE:'py2int', java.lang.Character.TYPE:'py2char'}
import JavaCall
...
class PyObject:
	...
	def asa(self, code, type, message=None):
		...
		tname = type.__name__
		tojava = jast.InvokeStatic('Py', 'tojava', [code, jast.StringConstant(tname)])
	...
	def igetitem(self, code, index):
		return self.domethod(code, "__getitem__", Object(jast.IntegerConstant(index), IntType))
	...
	def call(self, code, args, keyargs=None):
		...
		if keyargs is None or len(keyargs) == 0:
			if nargs == 0:
				...
				return self.domethod(code, "__call__", args[0], args[1], args[2])
			else:
			...
			return self.domethod(code, "__call__", PyObjectArray(args), makeStringArray(keynames))
...
types = {}
def findType(type):
	if types.has_key(type): return types[type]
