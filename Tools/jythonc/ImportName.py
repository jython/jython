...
from org.python.core import PyModule, PyJavaClass, PyClass, PyJavaPackage, PyBeanEventProperty, PyJavaDirPackage
	...
	try:
		...
		for d in path:
			...
			if os.path.exists(initfile):
				return Module(name=name, file=None, path=[os.path.join(d, name)])
	except "fake":
		return None
...
class Module(Namespace):
	def __init__(self, mod=None, name=None, file=None, path=None):
		...
		if file is None and hasattr(mod, '__file__'): file = mod.__file__
		if name is not None: self.name = name
		self.file = file
		if name is None and hasattr(mod, '__path__'): path = mod.__path__
		self.path = path
...
if __name__ == '__main__':
	import sys
	print sys.path
	print importName("pawt.colors"), importName("SimpleCompiler"), importName("pawt")
