...
class JavaVisitor(EmitVisitor):
    ...
    def sum_with_constructor(self, sum, name, depth):
        ...
        self.emit("public %(name)sType(PyType subType) {" % locals(), depth+1)
        self.emit("}", depth+1)
        self.emit("", 0)
    ...
    def visitConstructor(self, cons, name, depth):
        ...
        if ifaces:
            ...
            s = ""
        self.emit('@ExposedType(name = "_ast.%s", base = PyObject.class)' % cons.name, depth);
    ...
    def javaConstructors(self, type, clsname, ctorname, fields, depth):
        if len(fields) > 0:
            ...
            self.emit("public void Module___init__(PyObject[] args, String[] keywords) {}", depth)
