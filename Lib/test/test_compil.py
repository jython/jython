...
def compil(source,name="<input>",symbol="single"):
    return compile(source,name,symbol)

class CompilTests(unittest.TestCase):

    def assertValid(self, str, symbol='single',values=None,value=None):
        '''succeed iff str is a valid piece of code'''
        code = compil(str, "<input>", symbol)
        if values:
            d = {} 
            exec code in d
    ...
    def test_filename(self):
        self.assertEquals(compil("a = 1\n", "abc").co_filename,
                          compile("a = 1\n", "abc", 'single').co_filename)
        self.assertNotEquals(compil("a = 1\n", "abc").co_filename,
                             compile("a = 1\n", "def", 'single').co_filename)


def test_main():
    run_unittest(CompilTests)
