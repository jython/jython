 ...
 * formatting. In a limited way, it acts like a StringBuilder to
 * which text and one or more numbers may be appended, formatted
 * according to the format specifier supplied at construction. These
 * are ephemeral objects that are not, on their own, thread safe.
 */
public abstract class FloatFormatter extends InternalFormat.AbstractFormatter {

    /** The rounding mode dominant in the formatter. */
    static final RoundingMode ROUND_PY = RoundingMode.HALF_EVEN;

    /**
     * Limit the size of results. No-one needs more than
     * {@code log(Double.MAX_VALUE) - log2(Double.MIN_VALUE) = 1383}
     * digits.
     */
    static final int MAX_PRECISION = 1400;
    /**
     * If it contains no decimal point, this length is zero, and 1
     * otherwise.
     */
    ...
    /**
     * The length of the exponent marker ("e"), "inf" or "nan", or zero
     * if there isn't one.
     */
    private int lenMarker;
    /**
     * The length of the exponent sign and digits or zero if there isn't
     * one.
     */
    private int lenExponent;
    /**
     * If {@code minFracDigits&ge;0}, minimum digits to follow decimal
     * point (where consulted). When {@code minFracDigits&lt;0} this
     * signifies "no truncation" mode, in which trailing zeros generated
     * in the conversion are not removed.
     */
    ...
    public FloatFormatter(StringBuilder result, Spec spec) {
        ...
        if (spec.alternate) {
            ...
            /*
             * Every other format (if it does not ignore the setting) will by
             * default trim off all the trailing zero fractional digits.
             */
            minFracDigits = 0;
        }
    }
     ...
     * @param spec parsed conversion specification
     */
    public FloatFormatter(Spec spec) { this(new StringBuilder(size(spec)), spec); }

    /**
     * Recommend a buffer size for a given specification, assuming one
     * float is converted. This will be a "right" answer for e and
     * g-format, and for f-format with values up to 9,999,999.
     *
     * @param spec parsed conversion specification
     ...
     * trimming to zero or one. Set member {@code minFracDigits}, to
     * modify this behaviour.
     *
     * @param minFracDigits if &lt;0 prevent truncation; if &ge;0 the
     *     minimum number of fractional digits; when this is zero, and
     *     all fractional digits are zero, the decimal point will also
     *     be removed.
     */
    public void setMinFracDigits(int minFracDigits) { this.minFracDigits = minFracDigits; }
    ...
    /**
     * Format a floating-point number according to the specification
     * represented by this {@code FloatFormatter}.
     *
    ...
    /**
     * Format a floating-point number according to the specification
     * represented by this {@code FloatFormatter}. The conversion type,
     * precision, and flags for grouping or percentage are dealt with
    ...
    public FloatFormatter format(double value, String positivePrefix)
        ...
        /*
         * Puts all instance variables back to their starting defaults, and
         * start = result.length().
         */
        setStart();
        ...
        if (precision > MAX_PRECISION) { throw precisionTooLarge("float"); }

        /*
         * By default, the prefix of a positive number is "", but the format
         * specifier may override it, and the built-in type complex needs to
         * override the format.
         */
        char sign = spec.sign;
        ...
        switch (Character.toLowerCase(spec.type)) {
            ...
            case 'n':
                /*
                 * Locale-sensitive version of g-format should be here. (Désolé de
                 * vous decevoir.)
                 */
                /*
                 * XXX Set flag to signal localisation in/after groupDigits?
                 */
            case 'g':
                /*
            ...
            case '%':
                /*
                 * Multiplies by 100 and displays in f-format, followed by a percent
                 * sign.
                 */
        ...
        /*
         * If the format type is an upper-case letter, convert the result to
         * upper case.
         */
    ...
    /**
     * Convert just the letters in the representation of the current
     * number (in {@link #result}) to upper case. (That's the exponent
     * marker or the "inf" or "nan".)
     */
    @Override
    ...
    /**
     * Common code to deal with the sign, and the special cases "0",
     * "-0", "nan, "inf", or "-inf". If the method returns
     * {@code false}, we have started a non-zero number and the sign is
     * already in {@link #result}. The client need then only encode
     * <i>abs(value)</i>. If the method returns {@code true}, and
     * {@link #lenMarker}==0, the value was "0" or "-0": the caller may
     * have to zero-extend this, and/or add an exponent, to match the
     * requested format. If the method returns {@code true}, and
     * {@link #lenMarker}>0, the method has placed "nan, "inf" in the
     * {@link #result} buffer (preceded by a sign if necessary).
     *
     * @param value to convert
     * @return true if the value was one of "0", "-0", "nan, "inf", or
     *     "-inf".
     * @param positivePrefix to use before positive values (e.g. "+") or
     *     null to default to ""
     */
    private boolean signAndSpecialNumber(double value, String positivePrefix) {
        ...
        if ((bits & SIGN_MASK) != 0) {
            ...
            /*
             * Positive, and a prefix is required. Note CPython 2.7 produces
             * "+nan", " nan".
             */
        ...
        if (bits == 0L) {
            /*
             * All zero means it's zero. (It may have been negative, producing
             * -0.)
             */
            result.append('0');
            lenWhole = 1;
    ...
    /**
     * The e-format helper function of {@link #format(double, String)}
     * that uses Java's {@link BigDecimal} to provide conversion and
     * rounding. The converted number is appended to the {@link #result}
     * buffer, and {@link #start} will be set to the index of its first
     * character.
     *
     * @param value to convert
     * @param positivePrefix to use before positive values (e.g. "+") or
    ...
    private void format_e(double value, String positivePrefix, int precision) {
        ...
        if (!signAndSpecialNumber(value, positivePrefix)) {
            ...
            /*
             * Take explicit control in order to get exponential notation out of
             * BigDecimal.
             */
        ...
        /*
         * If the result is not already complete, add point and zeros as
         * necessary, and exponent.
         */
        if (lenMarker == 0) {
    ...
    /**
     * The f-format inner helper function of
     * {@link #format(double, String)} that uses Java's
     * {@link BigDecimal} to provide conversion and rounding. The
     * converted number is appended to the {@link #result} buffer, and
     * {@link #start} will be set to the index of its first character.
     *
     * @param value to convert
     * @param positivePrefix to use before positive values (e.g. "+") or
     *     null to default to ""
    ...
    private void format_f(double value, String positivePrefix, int precision) {
        ...
        if (!signAndSpecialNumber(value, positivePrefix)) {
            ...
            /*
             * Truncate to the defined number of places to the right of the
             * decimal point).
             */
            BigDecimal vv = vLong.setScale(precision, ROUND_PY);

            /*
             * When converted to text, the number of fractional digits is
             * exactly the scale we set.
             */
        ...
        if (lenMarker == 0) { ensurePointAndTrailingZeros(precision); }
    }

    /**
     * Append a decimal point and trailing fractional zeros if necessary
     ...
     * number of fractional digits specified in the argument {@code n},
     * and at least {@link #minFracDigits}; further, if
     * {@code minFracDigits&lt;0}, signifying the "alternate mode" of
     * certain formats, the method will ensure there is a decimal point,
     * even if there are no fractional digits to follow.
     *
    ...
    private void ensurePointAndTrailingZeros(int n) {
        ...
        if (n < minFracDigits) { n = minFracDigits; }

        // Do we have a decimal point already?
        if (lenPoint == 0) {
            /*
             * No decimal point: add one if there will be any fractional digits
             * (a defined number or "alternate mode").
             */
     ...
     * "internationalised"), and none-format (type code Spec.NONE).
     * <p>
     ...
     * presentation type {@code 'e'} and precision <i>p-1</i> would have
     * exponent exp. Then if <i>-4 &lt;= exp < p</i>, the number is
     * formatted with presentation type {@code 'f'} and precision
     * <i>p-1-exp</i>. Otherwise, the number is formatted with
     * presentation type {@code 'e'} and precision <i>p-1</i>. In both
     * cases insignificant trailing zeros are removed from the
     * significand, and the decimal point is also removed if there are
     * no remaining digits following it.
     * <p>
     * The Python documentation says none-format is the same as
     * g-format, but the observed behaviour differs from this, in that
     * f-format is only used if <i>-4 &lt;= exp < p-1</i> (i.e. one
     * less), and at least one digit to the right of the decimal point
     * is preserved in the f-format (but not the e-format). That
     * behaviour is controlled through the following arguments, with
     ...
     *     behaves as 1)
     * @param expThresholdAdj {@code +precision =} the exponent at which
     *     to resume using exponential notation
     */
    private void format_g(double value, String positivePrefix, int precision, int expThresholdAdj) {
        ...
        if (signAndSpecialNumber(value, positivePrefix)) {
            /*
             * Finish formatting if zero result. (This is a no-op for nan or
             * inf.)
             */
            ...
            /*
             * This gives us the digits we need for either fixed or exponential
             * format.
             */
    ...
    /**
     * Implementation of r-format ({@code float.__repr__}) that uses
     * Java's {@link Double#toString(double)} to provide conversion and
     * rounding. That method gives us almost what we need, but not quite
     * (sometimes it yields 18 digits): here we always round to 17
     * significant digits. Much of the formatting after conversion is
     * shared with {@link #format_g(double, String, int, int, int)}.
     * {@code minFracDigits} is consulted since while
     * {@code float.__repr__} truncates to one digit, within
     * {@code complex.__repr__} we truncate fully.
     *
     * @param value to convert
     * @param positivePrefix to use before positive values (e.g. "+") or
     *     null to default to ""
    ...
    private void format_r(double value, String positivePrefix) {
        ...
        /*
         * Characteristics of repr (precision = 17 and go exponential at
         * 16).
         */
        int precision = 17;
        ...
        if (signAndSpecialNumber(value, positivePrefix)) {
            ...
            /*
             * Generate digit sequence (with no decimal point) with custom
             * rounding.
             */
    ...
    /**
     * Common code for g-format, none-format and r-format called when
     * the conversion yields "inf", "nan" or zero. The method completes
     * formatting of the zero, with the appropriate number of decimal
     * places or (in particular circumstances) exponential; notation.
     *
     * @param precision of conversion (number of significant digits).
     * @param expThreshold if zero, causes choice of exponential
     *     notation for zero.
     */
    private void zeroHelper(int precision, int expThreshold) {
        ...
        if (lenMarker == 0) {
            ...
            /*
             * And just occasionally (in none-format) we go exponential even
             * when exp = 0...
             */
            if (0 >= expThreshold) { appendExponent(0); }
        }
    }

    /**
     * Common code for g-format, none-format and r-format used when the
     ...
     * point), in member {@code minFracDigits}, which is 0 for g-format
     * and 1 for none-format and r-format. When
     * {@code minFracDigits&lt;0} this signifies "no truncation" mode,
     * in which trailing zeros generated in the conversion are not
     * removed. This supports "%#g" format.
     *
     * @param digits from converting the value at a given precision.
     * @param exp would be the exponent (in e-format), used to position
     *     the decimal point.
     * @param precision of conversion (number of significant digits).
     */
    ...
    private void appendFixed(CharSequence digits, int exp, int precision) {
        ...
        /*
         * Check for "alternate format", where we won't economise trailing
         * zeros.
         */
        ...
        if (exp < 0) {
            ...
            result.append("0.");
            lenWhole = lenPoint = 1;
            for (int i = -1; i > exp; --i) { result.append('0'); }
            /*
             * Then the generated digits (always enough to satisfy no-truncate
             * mode).
             */
            ...
            /*
             * For a non-negative exponent, it's a question of placing the
             * decimal point.
             */
            int w = exp + 1;
            if (w < digitCount) {
                ...
                result.append(digits);
                /*
                 * Just occasionally (in r-format) we need more digits than the
                 * precision.
                 */
                while (digitCount < w) {
                    result.append('0');
        ...
        /*
         * Finally, ensure we have all and only the fractional digits we
         * should.
         */
        if (!noTruncate) {
     ...
     * presentation, when {@code minFracDigits&lt;0} this signifies "no
     * truncation" mode, in which trailing zeros generated in the
     * conversion are not removed. This supports "%#g" format.
     *
     * @param digits from converting the value at a given precision.
     * @param exp would be the exponent (in e-format), used to position
     *     the decimal point.
    ...
    private void appendExponential(CharSequence digits, int exp) {
        ...
        /*
         * In no-truncate mode, the fraction is full precision. Otherwise
         * trim it.
         */
        if (minFracDigits >= 0) {
            /*
     ...
     * {@code float.__repr__} (or r-format). This method takes advantage
     * of (or assumes) a close correspondence between
     * {@link Double#toString(double)} and Python
     * {@code float.__repr__}. The correspondence appears to be exact,
     * insofar as the Java method produces the minimal non-zero digit
     * string. It mostly chooses the same number of digits (and the same
     * digits) as the CPython repr, but in a few cases
     * {@code Double.toString} produces more digits. This method
     * truncates to the number {@code maxDigits}, which in practice is
     * always 17.
     *
     * @param value to convert
     * @param maxDigits maximum number of digits to return in
     *     {@code buf}.
    ...
    private static int reprDigits(double value, int maxDigits, StringBuilder buf) {
        ...
        while (p < end) {
            ...
            if (Character.isDigit(c)) {
                ...
                /*
                 * We remember this location (one *after* '.') to calculate the
                 * exponent later.
                 */
                point = p;
        ...
        /*
         * Possibly followed by an exponent. p has already advanced past the
         * 'E'.
         */
        if (p < end && c == 'E') {
            /*
             * If there is an exponent, the mantissa must be in standard form:
             * m.mmmm
             */
            ...
            /*
             * Exponent is based on relationship of decimal point and first
             * non-zero digit.
             */
            exp = point - first - 1;
            /*
             * But that's only correct when the point is to the right (or
             * absent).
             */
            if (exp < 0) {
        ...
        /*
         * XXX This still does not round in all the cases it could. I think
         * Java stops generating digits when the residual is <= ulp/2. This
         * is to neglect the possibility that the extra ulp/2 (before it
         * becomes a different double) could take us to a rounder numeral.
         * To fix this, we could express ulp/2 as digits in the same scale
         * as those in the buffer, and consider adding them. But Java's
         * behaviour here is probably a manifestation of bug JDK-4511638.
         */

        // Sometimes the result is more digits than we want for repr.
        if (buf.length() > maxDigits) {
            /*
             * Chop the trailing digits, remembering the most significant lost
             * digit.
             */
            int d = buf.charAt(maxDigits);
            buf.setLength(maxDigits);
            /*
             * We round half up. Not absolutely correct since Double has already
             * rounded.
             */
            if (d >= '5') {
                /*
                ...
                if (p < 0) {
                    /*
                     * We fell off the bottom of the buffer with one carry still to
                     * propagate. You may expect: buf.insert(0, '1') here, but note that
                     * every digit in buf[0:maxDigits] is currently '0', so all we need
                     * is:
                     */
                    buf.setCharAt(0, '1');
                    exp += 1;
    ...
    /**
     * Append the trailing fractional zeros, as required by certain
     * formats, so that the total number of fractional digits is no less
     * than specified. If {@code n&lt;=0}, the method leaves the
     * {@link #result} buffer unchanged.
     *
     * @param n smallest number of fractional digits on return
     */
     ...
     * Append the trailing fractional zeros, as required by certain
     * formats, so that the total number of fractional digits is no less
     * than specified. If there is no decimal point originally (and
     * therefore no fractional part), the method will add a decimal
     * point, even if it adds no zeros.
     *
     * @param n smallest number of fractional digits on return
     */
    private void appendPointAndTrailingZeros(int n) {
        ...
        // Now make up the required number of zeros.
        int f;
        for (f = lenFraction; f < n; f++) { result.append('0'); }
        lenFraction = f;
    }

    /**
     * Remove trailing zeros from the fractional part, as required by
     * certain formats, leaving at least the number of fractional digits
     * specified. If the resultant number of fractional digits is zero,
     * this method will also remove the trailing decimal point (if there
     * is one).
     *
     * @param n smallest number of fractional digits on return
     */
    private void removeTrailingZeros(int n) {
        ...
        if (lenPoint > 0) {
            /*
             * There's a decimal point at least, and there may be some
             * fractional digits.
             */
            int f = lenFraction;
            if (n == 0 || f > n) {
                ...
                if (f == 0 && lenPoint > 0) {
                    /*
                     * We will be stripping all the fractional digits. Take the decimal
                     * point too.
                     */
                ...
                if (fracStart + f < result.length()) { result.setLength(fracStart + f); }
            }
        }
    }

    /**
     * Append the current value of {@code exp} in the format
     * {@code "e{:+02d}"} (for example {@code e+05}, {@code e-10},
     * {@code e+308} , etc.).
     *
    ...
    /**
     * Return the index in {@link #result} of the first letter. This is
     * a helper for {@link #uppercase()} and {@link #getExponent()}
     */
    private int indexOfMarker() { return start + lenSign + lenWhole + lenPoint + lenFraction; }
