...
public class InternalFormat {
     ...
     * own, thread safe.
     */
    public static abstract class AbstractFormatter implements Appendable {
        ...
        /**
         * Signals the client's intention to make a {@code bytes} (or other
         ...
         * specification. Sets {@link #mark} and {@link #start} to the end
         * of the buffer. The new formatted object will therefore be
         * appended there and, when the time comes, padding will be applied
         * to (just) the new text.
         *
        ...
        /**
         * Construct the formatter from a specification and initial buffer
         * capacity. Sets {@link #mark} to the end of the buffer.
         *
         * @param spec parsed conversion specification
         * @param width of buffer initially
         */
        public AbstractFormatter(Spec spec, int width) { this(new StringBuilder(width), spec); }
        ...
        /**
         * Format an object into the buffer according to the specification
         * with which this instance has been initialised. The specific
         * behaviour, and the acceptable types, are for a sub-class to
         * define. Also, it is usually desirable for the return type in an
         * implementing class to be a more specific.
         *
         * @param o to format
         ...
         */
        public abstract AbstractFormatter format(Object o) throws NoConversion, FormatError;
        ...
        @Override
        public AbstractFormatter append(CharSequence csq, int start, int end)
         ...
         * {@link #result}, ready to receive a new one: sets {@link #start}
         * and calls {@link #reset()}. This is necessary when a
         * {@code Formatter} is to be re-used. Note that this leaves
         * {@link #mark} where it is. In the core, we need this to support
         * {@code complex}: two floats in the same format, but padded as a
         * unit.
         */
        ...
        /**
         * Clear the instance variables describing the latest object in
         * {@link #result}, ready to receive a new one. This is called from
         * {@link #setStart()}. Subclasses override this method and call
         * {@link #setStart()} at the start of their format method.
         */
         ...
         * relative to {@link #start}. The {@code toString} method shows a
         * '|' character between each section when it prints out the buffer.
         * Override this when you define more lengths in the subclass.
         *
         * @return the lengths of the successive sections
         */
        protected int[] sectionLengths() { return new int[] {lenSign, lenWhole}; }

        /**
         * {@inheritDoc}
         * <p>
         * Overridden to provide a debugging view in which the actual text
         * is shown divided up by the {@code len*} member variables. If the
         * dividers don't look right, those variables have not remained
        ...
        /**
         * Insert grouping characters (conventionally commas) into the whole
         * part of the number. {@link #lenWhole} will increase
         * correspondingly.
         *
        ...
        protected void groupDigits(int groupSize, char comma) {
            ...
            /*
             * Work out how many commas (or whatever) it takes to group the
             * whole-number part.
             */
            ...
            if (commasNeeded > 0) {
                /*
                 * Index *just after* the current last digit of the whole part of
                 * the number.
                 */
                ...
                /*
                 * The whole part will be longer by the number of commas to be
                 * inserted.
                 */
                lenWhole += commasNeeded;

                /*
                 * Now working from high to low, copy all the digits that have to
                 * move. Each pass copies one group and inserts a comma, which makes
                 * the to-pointer move one place extra. The to-pointer descends upon
                 * the from-pointer from the right.
                 */
                while (to > from) {
                    // Copy a group
        ...
        /**
         * Make a space in {@link #result} of a certain size and position.
         * On return, the segment lengths are likely to be invalid until the
         * caller adjusts them corresponding to the insertion. There is no
         * guarantee what the opened space contains.
         *
        ...
        /**
         * Pad the result so far (defined as the contents of {@link #result}
         * from {@link #mark} to the end) using the alignment, target width
         * and fill character defined in {@link #spec}. The action of
         * padding will increase the length of this segment to the target
         * width, if that is greater than the current length.
         * <p>
         * When the padding method has decided that that it needs to add n
         * padding characters, it will affect {@link #start} or
         ...
         * </tr>
         * </table>
         * Note that in the "pad after sign" mode, only the last number into
         * the buffer receives the padding. This padding gets incorporated
         * into the whole part of the number. (In other modes, the padding
         * is around {@code result[mark:]}.) When this would not be
        ...
        /**
         * Pad the last result (defined as the contents of {@link #result}
         * from argument {@code leftIndex} to the end) using the alignment,
         * by {@code n} repetitions of the fill character defined in
         * {@link #spec}, and distributed according to {@code spec.align}.
         * The value of {@code leftIndex} is only used if the alignment is
         * '&gt;' (left) or '^' (both). The value of the critical lengths
         * (lenWhole, lenSign, etc.) are not affected, because we assume
         * that {@code leftIndex &lt;= } {@link #start}.
         *
         * @param leftIndex the index in result at which to insert left-fill
         *     characters.
        ...
        protected void pad(int leftIndex, int n) {
            ...
            if (leading > 0) {
                ...
                for (int i = 0; i < leading; i++) { result.setCharAt(leftIndex + i, fill); }
            }
         ...
         * {@link #result} in the special case where a sign-aware padding
         * ({@code {@link #spec}.align='='}) was requested, the fill
         * character is {@code '0'}, and the digits are to be grouped. In
         * these exact circumstances, the grouping, which must already have
         * been applied to the (whole part) number itself, has to be
         * extended into the zero-padding.
         *
         * <pre>
         * &gt;&gt;&gt; format(-12e8, " =30,.3f")
         * '-            1,200,000,000.000'
         ...
         * </pre>
         *
         * The padding has increased the overall length of the result to the
         * target width. About one in three calls to this method adds one to
         * the width, because the whole part cannot start with a comma.
         *
         * <pre>
         * &gt;&gt;&gt; format(-12e8, " =30,.4f")
         * '-           1,200,000,000.0000'
        ...
        /**
         * Convenience method returning a {@link FormatError} reporting that
         * alternate form is not allowed in a format specifier for the named
         * type.
         *
         * @param forType the type it was found applied to
         * @return exception to throw
        ...
        /**
         * Convenience method returning a {@link FormatError} reporting that
         * the given alignment flag is not allowed in a format specifier for
         * the named type.
         *
        ...
        /**
         * Convenience method returning a {@link FormatError} reporting that
         * specifying a sign is not allowed in a format specifier for the
         * named type.
         *
         * @param forType the type it was found applied to
         * @param code the formatting code (or '\0' not to mention one)
        ...
        /**
         * Convenience method returning a {@link FormatError} reporting that
         * zero padding is not allowed in a format specifier for the named
         * type.
         *
         * @param forType the type it was found applied to
         * @return exception to throw
        ...
        /**
         * Convenience method returning a {@link FormatError} reporting that
         * specifying a precision is not allowed in a format specifier for
         * the named type.
         *
         * @param forType the type it was found applied to
         * @return exception to throw
        ...
        /**
         * Convenience method returning a {@link FormatError} reporting that
         * some format specifier feature is not allowed for the named data
         * type.
         *
         ...
         * Convenience method returning a {@link FormatError} reporting that
         * some format specifier feature is not allowed for the named data
         * type and format specifier {@code code}.
         *
         * @param outrage committed in the present case
         * @param forType the data type (e.g. "integer") where it is an
         *     outrage
         * @param code the formatting code for which it is an outrage (or
         *     '\0' not to mention one)
         * @return exception to throw
         */
        ...
        /**
         * Convenience method returning a {@link FormatError} reporting that
         * some format specifier feature is not allowed for the named format
         * code and data type. Produces a message like "OUTRAGE (SYMBOL) not
         * allowed with TYPE format specifier 'X'" or "OUTRAGE not allowed
         * in TYPE format specifier".
         *
         * @param outrage committed in the present case
         * @param symbol the modifier ('#' say) (or '\0' not to mention one)
         * @param forType the data type (e.g. "integer") it where it is an
         *     outrage
         * @param code the formatting code for which it is an outrage (or
         *     '\0' not to mention one)
         * @return exception to throw
         */
        public static FormatError notAllowed(String outrage, char symbol, String forType,
                char code) {
            // Try really hard to be like CPython
            // We may mention a symbol like # or ,
            String symbolAsString = symbol == 0 ? "" : " (" + symbol + ")";
            ...
            String msg = outrage + symbolAsString + " not allowed " + withOrIn + forType
                    + " format specifier" + codeAsString;
            return new FormatError(msg);
        }
    ...
    public static class Spec {
         ...
         * @return true only if the attribute is &ge;0 (meaning that it has
         *     been specified).
         */
        ...
        /**
         * Return a format specifier (text) equivalent to the value of this
         * Spec.
         */
        @Override
        public String toString() {
            ...
            if (specified(precision)) { buf.append('.').append(precision); }
            if (specified(type)) { buf.append(type); }
            return buf.toString();
        }

        /**
         * Return a merged {@code Spec} object, in which any attribute of
         * this object that is specified (or {@code true}), has the same
         * value in the result, and any attribute of this object that is
         * unspecified (or {@code false}), has the value that attribute
         * takes in the other object. Thus the second object supplies
         * default values. (These defaults may also be unspecified.) The use
         * of this method is to allow a {@code Spec} constructed from text
         * to record exactly, and only, what was in the textual
         * specification, while the __format__ method of a client object
         * supplies its type-specific defaults. Thus "20" means "&lt;20s" to
         * a {@code str}, "&gt;20.12" to a {@code float} and "&gt;20.12g" to
         * a {@code complex}.
         *
         * @param other defaults to merge where this object does not specify
         *     the attribute.
    ...
    private static class Parser {
        ...
        /**
         * Parse the specification with which this object was initialised
         * into an {@link Spec}, which is an object encapsulating the format
         * for use by formatting methods. This parser deals only with the
         * format specifiers themselves, as accepted by the
         * {@code __format__} method of a type, or the {@code format()}
         * built-in, not format strings in general as accepted by
         * {@code str.format()}.
         *
        ...
        Spec parse() {
            ...
            if (scanPast('0')) {
                /*
                 * Accept 0 here as equivalent to zero-fill but only not set
                 * already.
                 */
                if (!Spec.specified(fill)) {
                    ...
                    if (!Spec.specified(align)) {
                        /*
                         * Also accept it as equivalent to "=" alignment but only not set
                         * already.
                         */
                        align = '=';
                    }
        ...
        /**
         * Test that the next character is exactly the one specified, and
         * advance past it if it is.
         */
